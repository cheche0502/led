   ...
   //双循环配对
    ...
    for (int i = 0; i < lights.size(); i++) {
        for (int j = i + 1; j < lights.size(); j++) {
            ...
            // 右侧边界：右灯条中心往右扩展（0.8 比较合适经过测试，可以完全框住了）
...
//放弃拿角度找了，找不明白
            ...
            armor_corners = {
                cv::Point2f(left_x,top_y),     // 左上
                cv::Point2f(right_x,top_y),    // 右上
                cv::Point2f(right_x,bottom_y), // 右下
                cv::Point2f(left_x,bottom_y)   // 左下
            };    
            // 计算装甲板面积
    ...
    for (int i = 0;i<all_corners.size();i++) {
        ...
        if (corners_valid) {
            ...
            if (solveArmorPose(corners, rvec, tvec)) {
                ...
                cv::putText(image, dist_text, cv::Point(20, 30),cv::FONT_HERSHEY_SIMPLEX, 0.7,cv::Scalar(0, 255, 255),2);//PutText显示文本
...
// PnP位姿解算函数（3d—_>2d 的对应需要三部分内容：1.真实尺寸 2.四个点 3.相机内参）
                                 ...
                                 cv::Vec3d& rvec, cv::Vec3d& tvec)
{
    ...
    bool Armor_success = cv::solvePnP(armor_3d_points,ordered_corners,camera_matrix,dist_coeffs,rvec,tvec,false,cv::SOLVEPNP_EPNP);
