...
private:
    ...
    double LIGHT_MAX_RATIO = 25.0;  //灯条最大的长宽比，防止正方形和圆形灯条出现，如果这个值太小可能会导致细长的灯条出现
    double LIGHT_MIN_RATIO = 1.0;   //灯条最小长宽比
    double LIGHT_MAX_ANGLE = 80.0; //灯条最大角度，过滤掉横着的灯条，minAreaRect的角度范围是-90~0
    
    double MATCH_ANGLE_DIFF = 10.0;  //作为一个装甲板，两个灯条的方向基本一致，利用角度检验
    double MATCH_HEIGHT_RATIO = 3.0;   //确保高度相近，是利用高灯条除以矮灯条
    double MATCH_CENTER_DIST_RATIO = 10.0;   //两个灯条之间的最大允许距离 = 两个灯条的平均高度 × RATIO倍
    double MATCH_MIN_DISTANCE = 60.0; //灯条间最小距离，防止重叠灯条被误认为装甲板
